    ...
    pub fn load_dll<P: AsRef<Path>>(path: P) -> Result<Self, String> {
        ...
        unsafe {
            let lib = Library::new(path.as_ref())
                .map_err(|e| format!("Failed to load DLL {}: {}", path_str, e))?;

            // Extract Name
            let name_sym: libloading::Symbol<FfiPluginName> =
                lib.get(b"aura_rafi_plugin_name\0")
            ...
            let domain_sym: libloading::Symbol<FfiPluginDomain> = lib
                .get(b"aura_rafi_plugin_domain\0")
            ...
            let init_sym: libloading::Symbol<FfiInitPlugin> =
                lib.get(b"aura_rafi_plugin_init\0")
                    .map_err(|_| "Missing extern 'C' fn aura_rafi_plugin_init(api)")?;
            let update_sym: libloading::Symbol<FfiUpdatePlugin> =
                lib.get(b"aura_rafi_plugin_update\0").unwrap_or_else(|_| {
                    // Return a dummy symbol if not implemented, avoiding crashes.
                    let dummy: libloading::Symbol<FfiUpdatePlugin> =
                        std::mem::transmute(dummy_update as *const ());
                    dummy
                });
...
unsafe extern "C" fn c_api_submit_json_command(context: *mut c_void, json_str: *const c_char) {
    if context.is_null() || json_str.is_null() {
        return;
    }
    ...
    if let Ok(c_str) = CStr::from_ptr(json_str).to_str() {
        if let Ok(cmd) = serde_json::from_str::<Command>(c_str) {
            ...
            tracing::error!(
                "C++ Mod submitted invalid JSON Command structure: {}",
                c_str
            );
        }
    }
...
impl EngineComplement for CppComplement {
    ...
    fn on_init(&mut self, context: &mut ComplementContext<'_>) {
        ...
        unsafe {
            ...
            if !func(&api) {
                tracing::warn!(
                    "C++ Plugin '{}' initialization failed (returned false).",
                    self.name
                );
            }
        }
