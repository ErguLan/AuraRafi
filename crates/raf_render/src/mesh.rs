...
/// Face quads for a sphere (UV sphere, very low poly for performance).
/// `stacks` = latitude bands, `slices` = longitude segments.
/// At 4 stacks x 6 slices = 24 quads, runs on any hardware.
pub fn sphere_faces(stacks: usize, slices: usize) -> Vec<([Vec3; 4], Vec3)> {
    ...
    for i in 0..st {
        ...
        for j in 0..sl {
            ...
            // Normal = average of the 4 corner directions (cheap approximation).
...
/// Face quad for a unit plane (single quad, cannot get lighter).
...
/// Face quads for a cylinder. Side faces + top/bottom caps.
/// Uses `segments` quads for sides + 2 fan caps.
pub fn cylinder_faces(segments: usize) -> Vec<([Vec3; 4], Vec3)> {
    let r = 0.5;
    let h = 0.5;
    let seg = segments.max(6);
    let mut faces = Vec::with_capacity(seg * 2 + seg);
    ...
    for i in 0..seg {
        ...
        // Side face normal: average of the two radial directions.
        ...
        // Top cap triangle as degenerate quad (fan from center).
