...
impl ViewportPanel {
    ...
    // -------------------------------------------------------------------
    // 3D rendering (projected wireframes)
    // -------------------------------------------------------------------
    ...
    fn draw_3d(
        ...
        for (id, node) in scene.iter() {
            ...
            // Get wireframe edges for this primitive
            let edges = match node.primitive {
                Primitive::Cube => mesh::cube_edges(),
                Primitive::Sphere => mesh::sphere_edges(16),
                Primitive::Plane => mesh::plane_edges(),
                Primitive::Cylinder => mesh::cylinder_edges(16),
                Primitive::Sprite2D => mesh::plane_edges(), // flat quad
                Primitive::Empty => continue,
            };

            // Draw filled faces for cubes (simple flat shading)
            if node.primitive == Primitive::Cube {
                self.draw_cube_faces(painter, rect, &model, &view_proj, &node.color, is_selected);
            }
    ...
    fn draw_cube_faces(
        ...
        for (corners, normal) in mesh::cube_faces() {
            ...
            if !all_visible || screen_pts.len() < 4 {
                continue;
            }

            // Check if face is front-facing (simple 2D cross product)
            ...
            let fill = Color32::from_rgba_premultiplied(r, g, b, 180);
            ...
            // Use egui's PathShape for filled polygon
            let points = vec![screen_pts[0], screen_pts[1], screen_pts[2], screen_pts[3]];
    ...
    fn handle_input(&mut self, ui: &mut Ui, response: &egui::Response, rect: Rect) {
        ...
        ui.input(|i| {
            ...
            if i.key_pressed(egui::Key::F5) {
                // Handled by the mode toggle UI interaction
            }
        });
