...
impl ViewportPanel {
    ...
    pub(super) fn handle_object_mode_input(
        ...
        if response.drag_started_by(egui::PointerButton::Primary) {
            if let (Some(pos), Some(_entity_pos)) = (response.interact_pointer_pos(), selected_world_pos) {
            ...
            && self.bridge.active_drag_axis() == raf_render::gizmo::GizmoAxis::None
        {
            if let Some(pos) = response.interact_pointer_pos() {
                ...
                let picked = self.bridge.pick_entity(scene, view_proj, local_x, local_y, vp_w, vp_h);
                let add_to_selection = response.ctx.input(|i| i.modifiers.shift);

                if let Some(id) = picked {
                    if add_to_selection {
                        if let Some(existing) = self.selected.iter().position(|selected| *selected == id) {
