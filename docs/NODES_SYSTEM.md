...
## 5. Persistence and Prepared Runtime Binding
Node graphs are no longer editor-only state, but the product Play/runtime flow
is still guarded while the renderer and editor surfaces are stabilized.
...
- Runtime binding is prepared: every `On Start` entry is intended to execute
  once on runtime boot, and every `On Update` entry once per runtime frame.
- Console routing remains part of the planned reconnection pass.
