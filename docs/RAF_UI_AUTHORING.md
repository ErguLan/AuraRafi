...
For the full authoring-quality workflow, see
[RafUI Studio](../.ulpi/design/rafui-studio.md).

## RafUI Studio workflow

Use `RafUiStudio` when authoring or reviewing a surface. It keeps the repeated
editor vocabulary, inspection data, controlled edits, density matrix, golden
snapshots, and action truth checks in one place.

```rust
use raf_ui::{RafUiStudio, UiStudioCommandRegistry, UiStudioValidationOptions};

let studio = RafUiStudio::new([1280, 720]);
let inspection = studio.inspect(&document);
let diagnostics = studio.diagnose(&document);
let validation = studio.validate(
    &document,
    &UiStudioValidationOptions {
        command_registry: UiStudioCommandRegistry::from_names([
            "project.save",
            "viewport.grid",
        ]),
...
assert_eq!(inspection.first().map(|node| node.id.as_str()), Some("root"));
assert_eq!(diagnostics.node_count, inspection.len());
assert!(validation.document.node_count > 0);
```

The inspector is diagnostic data and can be rendered by a future RafUI
surface. It must not mutate scene/CAD state. `UiStudioEdit` is reserved for
document-authoring properties such as layout, style, labels, and tooltips.
